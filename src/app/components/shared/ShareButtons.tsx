...
const ShareButtons: React.FC<ShareButtonsProps> = ({ title, downloadUrl }) => {
    ...
    const shareLinks = [
        ...
        {
            name: "Instagram",
            label: "Share ke Instagram (salin link lalu buka IG)",
            ...
            directShare: false, // Instagram doesn't have public share API
        },
        {
            name: "X",
            label: "Share ke X (Twitter)",
            getUrl: () => {
                const url = currentUrl || (typeof window !== 'undefined' ? window.location.href : '');
        ...
        {
            name: "TikTok",
            label: "Share ke TikTok (salin link lalu buka TikTok)",
            getUrl: () => `https://www.tiktok.com/`,
            icon: (
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="currentColor"><path d="M19.59 6.69a4.83 4.83 0 0 1-3.77-4.25V2h-3.45v13.67a2.89 2.89 0 0 1-5.2 1.74 2.89 2.89 0 0 1 2.31-4.64 2.93 2.93 0 0 1 .88.13V9.4a6.84 6.84 0 0 0-1-.05A6.33 6.33 0 0 0 5 20.1a6.34 6.34 0 0 0 10.86-4.43v-7a8.16 8.16 0 0 0 4.77 1.52v-3.4a4.85 4.85 0 0 1-1-.1z" /></svg>
            ),
            color: "bg-black hover:bg-gray-800",
            directShare: false, // TikTok doesn't have public share API
        },
    ];
        ...
        <div className="flex flex-wrap items-center gap-3">
                ...
                {shareLinks.map((link) => {
                    ...
                    return (
                        <a
                            ...
                            onClick={(e) => {
                                e.preventDefault();
                                const shareUrl = getLinkUrl();

                                if (link.directShare) {
                                    // Platform supports direct share - open share dialog directly
                                    window.open(shareUrl, '_blank', 'width=600,height=400,scrollbars=yes');
                                } else {
                                    // Platform doesn't support direct share - copy link first then redirect
                                    navigator.clipboard.writeText(getShareMessage()).then(() => {
                                        alert(`✅ Link sudah disalin!\n\nPaste di ${link.name} untuk membagikan.`);
                                        window.open(shareUrl, '_blank');
                                    });
                                }
                            }}
                            className={`${link.color} text-white w-8 h-8 flex items-center justify-center rounded-full transition-transform hover:-translate-y-1 shadow-sm`}
                            title={link.label}
            ...
            <div className="flex gap-2">
                <button
                    ...
                    className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold transition-all shadow-sm ${copied ? 'bg-green-600 text-white' : 'bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-600'}`}
                    title="Copy Link"
                >
                    {copied ? (
                        <>
                            <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="currentColor"><path d="M20.285 2l-11.285 11.567-5.286-5.011-3.714 3.716 9 8.728 15-15.285z" /></svg>
                            <span>Tersalin</span>
                        </>
                    ) : (
                        <>
                ...
                {downloadUrl && (
                    <button
                        ...
                        className="flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold transition-all shadow-sm bg-[#1a56db] hover:bg-blue-700 text-white"
