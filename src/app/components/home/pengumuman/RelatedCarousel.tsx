...
const RelatedCarousel: React.FC<RelatedCarouselProps> = ({ items }) => {
    const settings = {
        dots: true,
                    ...
                    <div key={item.id} className="px-3 pb-8">
                        <div className="bg-white/10 backdrop-blur-md border border-white/20 rounded-2xl overflow-hidden shadow-lg h-full hover:shadow-xl hover:bg-white/15 transition-all duration-300">
                            ...
                            <div className="p-5">
                                ...
                                <Link href={`/pengumuman/${item.url_halaman}`} className="text-blue-300 hover:text-blue-200 text-sm font-semibold hover:underline inline-flex items-center gap-1">
                                    Baca Selengkapnya
